...
let ltmc_thread_nba ((_, frs, _, vars) as dcp) =
									...
									FIntVariable z' ->
										if z = z' then (
											...
											let (_, (_, prv, _, _)) = vars.(v) in
												if (prz <= prv) then [Tracking (g, v)] else [Failed]
										)
										else [s]
									| _ -> [s]
								)
...
let ltmc_thread_nba_state_size (_, frs, _, vars) = 1 + (Array.length frs) * (Array.length vars);;
