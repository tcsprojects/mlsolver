...
let get_modelchecking_game (((r, fmls, props, vars, lbls) as fml): decomposed_labelled_mmc_formula)
...
let identify_step_state (_, fmls, _, _, _) = function
	Atom _ -> true
|	Tuple (formula, _) ->
		match fmls.(formula) with
...
let modelchecking_proc formula in_chan options (info_chan, formula_chan, constr_chan) =
	...
	info_msg (fun _ -> "Modelchecking Procedure For Labelled Modal Mu Calculus\n");
	...
	let t = SimpleTiming.init true in
	let lts = Tcstransitionsysparser.parse_explicit_initlts in_chan in
	constr_msg (fun _ -> SimpleTiming.format t ^ "\n");
	
	let states_game = ref 0 in

	constr_msg (fun _ -> "Initializing game...");
	let game = get_modelchecking_game decformula lts in
