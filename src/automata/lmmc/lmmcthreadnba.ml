...
let lmmc_thread_nba ((_, frs, _, vars, _) as dcp) =
	...
	let delta s r =
		...
		|	Tracking (f, v) -> (
				...
				|	Follow i -> (
						...
						|	FIntModality (false, l, n) -> (
								match frs.(f) with
									FIntModality(b, l', n') ->
										if (l = l') && (b || (n = n')) then [Tracking (n', v)] else [Failed]
								| _ -> [Failed]
							)
						|	FIntVariable z -> (
								match frs.(f) with 
...
let lmmc_thread_nba_state_size (_, frs, _, vars, _) = 1 + (Array.length frs) * (Array.length vars);;
